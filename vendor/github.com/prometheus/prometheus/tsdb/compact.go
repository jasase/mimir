...
type Compactor interface {
	...
	Write(dest string, b BlockReader, mint, maxt int64, parent *BlockMeta, shard func(labels.Labels) uint64) (ulid.ULID, error)
	...
	Compact(dest string, dirs []string, open []*Block, shard func(labels.Labels) uint64) (ulid.ULID, error)
	...
	CompactOOO(dest string, oooHead *OOOCompactionHead, shard func(labels.Labels) uint64) (result []ulid.ULID, err error)
}
...
func (c *LeveledCompactor) CompactWithSplitting(dest string, dirs []string, open []*Block, shardCount uint64, shard func(labels.Labels) uint64) (result []ulid.ULID, _ error) {
	return c.compact(dest, dirs, open, shardCount, shard)
}

// Compact creates a new block in the compactor's directory from the blocks in the
// provided directories.
func (c *LeveledCompactor) Compact(dest string, dirs []string, open []*Block, shard func(labels.Labels) uint64) (uid ulid.ULID, err error) {
	ulids, err := c.compact(dest, dirs, open, 1, shard)
	if err != nil {
...
func (c *LeveledCompactor) compact(dest string, dirs []string, open []*Block, shardCount uint64, shard func(labels.Labels) uint64) (_ []ulid.ULID, err error) {
	if shardCount == 0 {
		shardCount = 1
	}
	...
	err = c.write(dest, outBlocks, shard, blocks...)
	if err == nil {
...
func (c *LeveledCompactor) CompactOOOWithSplitting(dest string, oooHead *OOOCompactionHead, shardCount uint64, shard func(labels.Labels) uint64) (result [][]ulid.ULID, _ error) {
	return c.compactOOO(dest, oooHead, shardCount, shard)
}

// CompactOOO creates a new block per possible block range in the compactor's directory from the OOO Head given.
// Each ULID in the result corresponds to a block in a unique time range.
func (c *LeveledCompactor) CompactOOO(dest string, oooHead *OOOCompactionHead, shard func(labels.Labels) uint64) (result []ulid.ULID, err error) {
	ulids, err := c.compactOOO(dest, oooHead, 1, shard)
	if err != nil {
		return nil, err
	}
	for _, s := range ulids {
...
func (c *LeveledCompactor) compactOOO(dest string, oooHead *OOOCompactionHead, shardCount uint64, shard func(labels.Labels) uint64) (_ [][]ulid.ULID, err error) {
	if shardCount == 0 {
		shardCount = 1
	}
	...
	for t := blockSize * (oooHeadMint / blockSize); t <= oooHeadMaxt; t = t + blockSize {
		...
		err := c.write(dest, outBlocks[ix], shard, oooHead.CloneForTimeRange(mint, maxt-1))
		if err != nil {
...
func (c *LeveledCompactor) Write(dest string, b BlockReader, mint, maxt int64, parent *BlockMeta, shard func(labels.Labels) uint64) (ulid.ULID, error) {
	start := time.Now()
	...
	err := c.write(dest, []shardedBlock{{meta: meta}}, shard, b)
	if err != nil {
		return uid, err
	}
...
func (c *LeveledCompactor) write(dest string, outBlocks []shardedBlock, shard func(labels.Labels) uint64, blocks ...BlockReader) (err error) {
	...
	if err := c.populateBlock(blocks, outBlocks[0].meta.MinTime, outBlocks[0].meta.MaxTime, outBlocks, shard); err != nil {
...
func (c *LeveledCompactor) populateBlock(blocks []BlockReader, minT, maxT int64, outBlocks []shardedBlock, shard func(labels.Labels) uint64) (err error) {
	if len(blocks) == 0 {
	...
	if len(outBlocks) == 1 {
		...
		if err := c.populateSymbols(symbolsSets, outBlocks, shard); err != nil {
			return err
		}
	}
	...
	for set.Next() {
		...
		obIx := uint64(0)
		if len(outBlocks) > 1 {
			obIx = shard(s.Labels()) % uint64(len(outBlocks))
		}
...
// block's index.
func (c *LeveledCompactor) populateSymbols(sets []storage.ChunkSeriesSet, outBlocks []shardedBlock, shard func(labels.Labels) uint64) error {
	if len(outBlocks) == 0 {
	...
	for seriesSet.Next() {
		...
		err := s.Labels().RangeToError(func(l labels.Label) error {
